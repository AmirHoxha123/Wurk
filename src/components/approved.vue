<template>
  <div>
    ...
    <q-list class="q-pt-md q-pl-lg q-ma-xl">
      ...
      <q-list style="margin-bottom: 20px; border-radius: 30px;" v-for="row in approvedRows" :key="row" class="bg-white q-ml-lg">
          ...
          <q-item-section style="margin-right: 10px">
            <div class="row">
              ...
              <q-btn v-else class="col bg-red-6" style="cursor: pointer; color: white; margin-right: 10px;"
                     no-caps label="Decline" @click="declineBtn(row.id)" padding="xs xl"
              />
    ...
    <div v-if="approvedRows.length < 1" class="q-pt-xl" style="margin: auto; width: 10%;">
      No Data
    </div>
    <q-dialog v-model="showTheEditDialog" persistent>
      <q-card style="width: 700px; max-width: 80vw;">
        <div v-if="approvedLoading">
          <q-inner-loading
              showing
              color="secondary"
          />
...
export default {
  ...
  methods:{
    ...
    getApproved(){
      this.loading = true
      axios.get(`${baseUrl}?status=APPROVED`,  { headers })
          .then((res) => {
            this.approvedRows = res.data.map(obj => {
    ...
    declineBtn(id){
      ...
      axios.put(`https://dev.api.wurk.skyver.co/api/workingtimeframes/${id}/status?status=DECLINED`,
          {}, {headers})
          .then(res => {
            if (res.status === 204) {
